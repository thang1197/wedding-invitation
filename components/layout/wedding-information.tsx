...
import { Faustina, Corinthia } from "next/font/google";
import styles from "./style.module.css";
import { useEffect, useRef } from "react";
import { usePathname } from "next/navigation";

const faustina = Faustina({ subsets: ["latin"], weight: "400" });
const corinthia = Corinthia({weight: '700', subsets: ["latin"]});
...
export function WeddingInformation() {
  ...
  const guest = atob(pathName);

  useEffect(() => {
    if (sectionRef.current && titleRef.current && addressRef.current && mapRef.current) {
  ...
  }, [sectionRef.current, titleRef.current, addressRef.current, mapRef.current]);
      ...
      <div className="max-w-4xl mx-auto">
        ...
        >
          ...
          <div ref={titleRef} className={`${styles["wedding-info-title"]}`}>
            ...
            <h3 className={`font-bold font-serif text-5xl ${corinthia.className}`}>{guest}</h3>
          ...
          className={`${styles['address']}`}>
            <h3 className="uppercase text-2xl underline font-serif">
              Địa điểm
            </h3>
